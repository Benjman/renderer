...
class Text {
    public:
        static inline constexpr size_t ELEMENTS_PER_VERT = 16;
        static inline constexpr size_t ELEMENTS_PER_INDEX = 6;
    ...
    public:
        ...
        void calc_buffer_sizes(size_t* vert_buf_size, size_t* idx_buf_size) const;

        size_t generate_mesh(float_t *vert_buf, uint32_t *idx_buf, float_t display_width, float_t display_height, size_t idx_cursor = 0) const;

        size_t generate_mesh(TextMeshContext& context) const;

        void update_metadata();

        float_t get_font_scale() const;

        size_t renderable_quad_count = 0;
