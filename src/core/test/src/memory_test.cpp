...
TEST(MemoryTest, initial_state) {
    MemoryZone<T> zone(nullptr, MAX_SIZE);
...
TEST(MemoryTest, with_parent) {
    size_t size = 8;
    MemoryZone<T> parent(nullptr, MAX_SIZE);
    MemoryZone<T> child(&parent, size);
...
TEST(MemoryTest, with_parent_many_children) {
    size_t size = 8;
    MemoryZone<T> parent(nullptr, MAX_SIZE);
    MemoryZone<T> child1(&parent, size);
    MemoryZone<T> child2(&parent, size);
...
TEST(MemoryTest, reclaim_tail) {
    MemoryZone<T> parent(nullptr, 24);
    MemoryZone<T> child1(&parent, 8);
    ASSERT_EQ(8, parent.cursor);
    MemoryZone<T> child2(&parent, 8);
    ASSERT_EQ(16, parent.cursor);
    MemoryZone<T> child3(&parent, 8);
